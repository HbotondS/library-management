...
for (var key in json) {
  ...
  genres = db.Genre.aggregate([
    { $sample: { size: nrGenres } }, 
    { $project: { _id: 0, name: 1 } }
  ]).toArray()
    .map((value) => value.name);

  publisher = db.Publisher.aggregate([
    { $sample: { size: 1 } }, 
    { $project: { _id: 1 } }
  ]).toArray()[0]['_id'];

  translater = db.People.aggregate([
    { $sample: { size: 1 } },
  ...
  db.Book.insertOne({
    ...
    publishedDate: new Date(new Date() - Math.random() * 1e12),
    isbn: json[key].isbn,
    translater: {
